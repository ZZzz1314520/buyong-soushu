...
- `sources` — configured search engines (list of `SearchSource`)
...
- **Search**: `SearchScreen` → `controller.search(query)` → `NovelService.searchBooks()` → fetches from each enabled `SearchSource` in parallel, de-duplicates by URL, returns up to 50 results.
...
5. **Search parsing**: Scores links by query relevance, filters out search-engine hostnames (bing, google, baidu, etc.) and navigation text.
...
- `SearchSource` — search engine config with `{query}` URL template and enable/disable toggle. Two built-in defaults: Bing and DuckDuckGo.
...
- `ReaderSettings` — font size, line height, `ReaderTheme` enum (paper/green/dark/pureWhite), `PageTurnMode` enum (verticalScroll/tapSides).
...
- Reading settings bottom sheet (font size slider, line height slider, theme segments, page-turn mode segments)
- Chapter catalog bottom sheet (tap to jump)
- Previous/next chapter buttons in bottom bar
- Tap-sides mode: left half of screen = previous, right half = next
